...
export function formatSignificantNumber(number) {
  ...
  var magnitude = Math.pow(10, 4 - Math.floor(Math.log10(Math.abs(number))) - 1);
...
export const formatPrice = (coinPerc, totalBal) => {
  return new BigNumber(+coinPerc / 100).times(new BigNumber(totalBal)).decimalPlaces(2, BigNumber.ROUND_DOWN).toNumber();
};
...
export function applyDecimals(number, decimals) {
  return formatSignificantNumber(BigNumber(number).dividedBy(10 ** decimals));
}
